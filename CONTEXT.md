...
**Duplicate group**:
The stable, opaque, versioned id (format `cr2-<hash>`) the route assigns to every surviving member of one Group. Deterministic over the sorted member `source_uid`s, so it changes only when membership changes; no consumer may persist it as identity (picks stay on `event_id` plus the `merged_ids` union).
