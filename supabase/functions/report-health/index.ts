...
export async function processHealthReport(
  supabase: any,
  payload: { city: string; feeds: any[]; anomalies?: any[] },
): Promise<any> {
  const { city, feeds, anomalies } = payload;

  if (!city || !Array.isArray(feeds)) {
    return { success: false, error: "Missing required fields: city, feeds" };
  }
...
export const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const result = await processHealthReport(supabase, body);

    if ("error" in result && result.success === false && Object.keys(result).length === 2) {
      // Validation error from processHealthReport
