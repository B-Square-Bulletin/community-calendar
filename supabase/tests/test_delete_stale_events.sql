...
SELECT is(
  (SELECT count(*)::int FROM events WHERE city = 'test_dse'),
  3,
  'Test 1b: 3 events remain after stale deletion'
);

SELECT is(
  (SELECT count(*)::int FROM events WHERE city = 'test_dse' AND source_uid = 'uid-a'),
  1,
  'Test 1c: uid-a is still present'
);

-- Cleanup for next test
DELETE FROM events WHERE city = 'test_dse';
...
SELECT is(
  (SELECT count(*)::int FROM events WHERE city = 'test_dse' AND source_uid = 'uid-survivor'),
  1,
  'Test 4b: Survivor event still present after large-array delete_stale_events'
);
