...
describe('BrandHeader', () => {
  it('header exists', () => { expect(!!header).toBe(true); });
  it('header links logo to bsquarebulletin.com same-tab', () => { expect(!!header && header.includes('https://bsquarebulletin.com/') && !/to="https:\/\/bsquarebulletin\.com[^"]*"[^>]*target="_blank"/.test(header)).toBe(true); });
  it('header sub-label uses $color-text-primary (dark-tone safe), bold, letter-spaced', () => { expect(!!header && header.includes('Community Calendar')
  && header.includes('$color-text-primary')
  && !/color\s*=\s*["']black["']/.test(header)
  && header.includes('$fontWeight-bold') && /letterSpacing/i.test(header)).toBe(true); });
  it('header logo pinned in a light-tone island (single asset, no tone tracking)', () => { expect(!!header && /<Theme[^>]*tone="light"/.test(header)
  && LOGO_REF.test(header)).toBe(true); });
  it('header island paints a pinned-light badge behind the logo', () => { expect(!!header && /<Theme[^>]*tone="light"[\s\S]*?backgroundColor="white"[\s\S]*?BSB_Logo-2-color-horiz\.svg[\s\S]*?<\/Theme>/.test(header)).toBe(true); });
  it('header 4px red bottom rule', () => { expect(!!header && /borderBottom="4px solid \$color-primary"/.test(header)).toBe(true); });
  it('header Inter chrome font scoped to component', () => { expect(!!header && header.includes('Inter')).toBe(true); });
  it('header hidden on embed', () => { expect(!!header && header.includes('!window.embed')).toBe(true); });
  it('header references vendored logo with APP_VERSION', () => { expect(!!header && LOGO_REF.test(header)).toBe(true); });
});

describe('BrandFooter', () => {
  it('footer exists', () => { expect(!!footer).toBe(true); });
  it('footer red bar white text', () => { expect(!!footer && footer.includes('$color-primary') && footer.includes('$color-surface')).toBe(true); });
  it('footer Chronicle link new-tab', () => { expect(!!footer && footer.includes('https://thebloomingtonchronicle.org/index.php/Main_Page') && /thebloomingtonchronicle[^>]*target="_blank"/.test(footer)).toBe(true); });
  it('footer BloomDocs link new-tab', () => { expect(!!footer && footer.includes('https://bloomdocs.org/') && /bloomdocs\.org[^>]*target="_blank"/.test(footer)).toBe(true); });
  it('footer Contact link same-tab', () => { expect(!!footer && footer.includes('https://bsquarebulletin.com/contact-the-b-square/')
  && !/contact-the-b-square[^>]*target="_blank"/.test(footer)).toBe(true); });
  it('footer Republishing link same-tab', () => { expect(!!footer && footer.includes('https://bsquarebulletin.com/republishing-guidelines/')
  && !/republishing-guidelines[^>]*target="_blank"/.test(footer)).toBe(true); });
  it('footer has no Ghost portal links', () => { expect(!!footer && !footer.replace(/<!--[\s\S]*?-->/g, '').includes('#/portal/')).toBe(true); });
  it('footer copyright line', () => { expect(!!footer && footer.includes('© The B Square. All rights reserved.')).toBe(true); });
  it('footer Inter chrome font', () => { expect(!!footer && footer.includes('Inter')).toBe(true); });
  it('footer hidden on embed', () => { expect(!!footer && footer.includes('!window.embed')).toBe(true); });
});

describe('logo asset', () => {
  it('logo vendored under icons/', () => { expect(!!logo && logo.includes('<svg')).toBe(true); });
  it('logo has no scripts', () => { expect(!!logo && !/script|onload|onclick/i.test(logo)).toBe(true); });
  it('logo wordmark stays near-black (asset untouched by tone fix)', () => { expect(!!logo && /fill:#231f20/i.test(logo)).toBe(true); });
  it('logo red square unchanged (brand red, no filter shift)', () => { expect(!!logo && /fill:#d21c2d/i.test(logo)).toBe(true); });
  it('logo registered in config.json resources', () => { expect(!!config && config.includes('BSB_Logo-2-color-horiz.svg')).toBe(true); });
});
...
describe('Main wiring', () => {
  ...
  it('header included in all 3 standalone blocks (picker,list,dashboard)', () => { expect(headerIncludes === 3).toBe(true); });
  it('footer included in all 3 standalone blocks (picker,list,dashboard)', () => { expect(footerIncludes === 3).toBe(true); });
  it('dashboard loading state keeps chrome (not gated on tiles)', () => { expect(!!main && /when="\{layoutMode === 'dashboard'( && !window\.embed)?\}"/.test(main)
  && !/layoutMode === 'dashboard' && \(dashboardTiles !== null\)[^>]*Brand(Header|Footer)/.test(main)).toBe(true); });
  it('picker redundant H1 dropped', () => { expect(!!main && !/<H1>Community Calendar<\/H1>/.test(main)).toBe(true); });
  it('city-name heading intact', () => { expect(!!main && main.includes('{window.toDisplayName(city)}')).toBe(true); });
  it('title-row controls (IconRow) intact', () => { expect(!!main && (main.match(/components\/IconRow\.xmlui/g) || []).length === 2).toBe(true); });
  it('no fontFamily overrides on existing components', () => { expect(!!main && !/fontFamily/.test(main)
  && (() => {
...
describe('theme/boot', () => {
  it('theme surfaces are warm-neutral (not pure gray)', () => { expect(!!themeText && (() => {
    try {
  ...
  it('index.html version-busts boot-decisions.js before shell.js', () => { expect(!!index && index.includes('boot-decisions.js')
  && !/<script src="boot-decisions\.js"><\/script>/.test(index)
  && index.indexOf('src="boot-decisions.js') < index.indexOf('src="shell.js')).toBe(true); });
});

describe('content links', () => {
  it('theme keeps chrome red primary (BSB brand)', () => { expect(!!themeText && (() => {
    try {
      return JSON.parse(themeText).themeVars['color-primary'] === 'hsl(354, 75%, 47%)';
    } catch { return false; }
  })()).toBe(true); });
  it('theme decouples content links to darker maroon (not chrome red)', () => { expect(!!themeText && (() => {
    try {
  ...
  it('content link color meets WCAG AA 4.5:1 on white card + warm surface', () => { expect(!!themeText && (() => {
    try {
      const vars = JSON.parse(themeText).themeVars;
  ...
  it('event title inherits theme link color (no per-instance override)', () => { expect(!!read('components/EventCard.xmlui') && (() => {
...
describe('sub-label contrast', () => {
  it('sub-label light token defined + meets AA on light surfaces', () => { expect(!!themeText && (() => {
  ...
  it('sub-label dark token defined + meets AA on dark surface', () => { expect(!!themeText && (() => {
