...
describe('empty labels', () => {
  ...
  it('empty labels for day presets', () => {
    expect(['today', 'tomorrow'].map((p) => window.dateWindowLabel(p))).toEqual(['today', 'tomorrow']);
  });
...
describe('truncation copy', () => {
  ...
  it('truncation copy names the inclusive last day', () => {
    expect(window.dateTruncationText(monthCut)).toEqual('Showing through 2026-02-14 — the calendar currently ends there.');
  });
...
describe('URL contract', () => {
  beforeEach(() => {
    global.window.location = new URL('https://example.com/?city=davis&embed=true&search=x&category=y&mode=list&images=preview&cards=25');
  });
  it('preset commit preserves embed and every sibling', () => {
    window.syncDateParams({ preset: 'today' });
    const u = new URL(window.location);
    expect([u.searchParams.get('date'), u.searchParams.get('embed'), u.searchParams.get('city'),
      u.searchParams.get('search'), u.searchParams.get('category'), u.searchParams.get('mode'),
      u.searchParams.get('images'), u.searchParams.get('cards')]).toEqual(['today', 'true', 'davis', 'x', 'y', 'list', 'preview', '25']);
  });
  it('All strips only date keys and preserves city, search, category, display params', () => {
    window.syncDateParams({ preset: 'today' });
    window.syncDateParams({ preset: 'all' });
    const u = new URL(window.location);
    expect([u.searchParams.get('date'), u.searchParams.get('from'), u.searchParams.get('to'),
      ...
      u.searchParams.get('cards')]).toEqual([null, null, null, 'davis', 'x', 'y', 'list', 'preview', 'true', '25']);
  });
  it('custom commit preserves embed alongside the exact dates', () => {
    ...
    expect([u.searchParams.get('from'), u.searchParams.get('to'), u.searchParams.get('date'),
      u.searchParams.get('embed'), u.searchParams.get('city')]).toEqual(['2026-02-12', '2026-02-14', null, 'true', 'davis']);
  });
});

describe('invalid-link fallbacks', () => {
  it('unknown preset key falls back to All', () => {
    expect(window.decodeDateParams({ date: 'bogus' }, { timeZone: 'America/Los_Angeles' }).preset).toEqual('all');
  });
  it('malformed custom pair falls back to All', () => {
    expect(window.decodeDateParams({ from: '2026-02-30', to: '2026-03-01' }, { timeZone: 'America/Los_Angeles' }).preset).toEqual('all');
  });
  it('partial custom pair falls back to All', () => {
    expect(window.decodeDateParams({ from: '2026-02-12' }, { timeZone: 'America/Los_Angeles' }).preset).toEqual('all');
  });
  it('invalid pair is ignored and the valid preset wins', () => {
    expect(window.decodeDateParams({ date: 'today', from: 'bogus', to: '2026-02-14' }, { timeZone: 'America/Los_Angeles' }).preset).toEqual('today');
  });
  it('partial pair is ignored and the valid preset wins', () => {
    expect(window.decodeDateParams({ date: 'today', from: '2026-02-12' }, { timeZone: 'America/Los_Angeles' }).preset).toEqual('today');
  });
  it('valid pair still wins over the preset key', () => {
    expect(window.decodeDateParams({ date: 'today', from: '2026-02-12', to: '2026-02-14' }, { timeZone: 'America/Los_Angeles' }).preset).toEqual('custom');
  });
  it('inverted URL pair is ignored (All when no preset)', () => {
    expect(window.decodeDateParams({ from: '2026-09-10', to: '2026-09-05' }, { timeZone: 'America/Los_Angeles' }).preset).toEqual('all');
  });
  it('inverted URL pair defers to the valid preset', () => {
    expect(window.decodeDateParams({ date: 'today', from: '2026-09-10', to: '2026-09-05' }, { timeZone: 'America/Los_Angeles' }).preset).toEqual('today');
  });
  it('picker path still coerces an inverted pair to the single from-day', () => {
    expect(window.resolveCustomRange('2026-09-10', '2026-09-05', { timeZone: 'America/Los_Angeles' }).to).toEqual('2026-09-10');
  });
...
describe('Tonight exclusions', () => {
  ...
  it('without the flag the same window keeps unflagged-shape starts (no behavior change)', () => {
    expect(window.filterByDateWindow(
      ...
      TONIGHT_START, TONIGHT_END).map((e) => e.id)).toEqual(['show']);
  });
});

describe('single-path parity', () => {
  it('helpers truncation delegates to the engine formatter byte-identically', () => {
    expect(window.dateTruncationText(monthCut)).toEqual(window.truncationLabelForWindow(monthCut, 'America/Los_Angeles'));
  });
  it('engine formatter names the inclusive last day (hand-computed PST)', () => {
    expect(window.truncationLabelForWindow(monthCut, 'America/Los_Angeles')).toEqual('Showing through 2026-02-14 — the calendar currently ends there.');
  });
  it('engine formatter is null without an overrun', () => {
    expect([window.truncationLabelForWindow(month, 'America/Los_Angeles'),
      window.truncationLabelForWindow({ start: null, end: null, truncated: false }, 'America/Los_Angeles')]).toEqual([null, null]);
  });
  it('single copy constant owns the truncation wording', () => {
    expect([window.TRUNCATION_LABEL_PREFIX, window.TRUNCATION_LABEL_SUFFIX]).toEqual(['Showing through ', ' — the calendar currently ends there.']);
  });
  ...
  it('helpers preset+custom resolvers share one opts builder', () => {
    expect([typeof window.dateWindowOpts, JSON.stringify(Object.keys(window.dateWindowOpts()).sort())]).toEqual(['function', JSON.stringify(['horizonEnd', 'timeZone'])]);
  });
});
...
describe('naming', () => {
  ...
  it('no single-letter window/date locals on the date path', () => {
    expect(Object.entries(dateSources).map(([name, src]) => [name, !hasDateSingleLetter(src)])).toEqual([['date-windows.js', true], ['helpers.js', true], ['shell.js', true], ['Globals.xs', true], ['Main.xmlui', true]]);
  });
});
