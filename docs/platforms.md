...
| **Simpleview CMS**                | Tourism sites. No usable ICS/RSS (RSS caps at 30 date-only items) and no drop-in feed, but a same-origin token+JSON REST API (`/includes/rest_v2/plugins_events_events_by_date/find/`, token from `/plugins/core/get_simple_token/`) returns clock times, descriptions, geo and recurrence over plain HTTP. Needs a custom scraper per site and a realistic desktop-Chrome UA (Akamai 403s the default `requests` UA and `limit >= 64`)                                                                                                                                                                                                                                                                                                                               |
