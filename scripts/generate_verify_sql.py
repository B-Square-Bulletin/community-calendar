...
def parse_ddl_files():
    ...
    for path in sorted(DDL_DIR.glob("*.sql")):
        ...
        for m in re.finditer(
            r"CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(?:public\.)?(\w+)", text, re.IGNORECASE
        ):
        ...
        # Functions
        for m in re.finditer(
            r"CREATE\s+(?:OR\s+REPLACE\s+)?FUNCTION\s+(?:public\.)?(\w+)", text, re.IGNORECASE
        ):
            functions[m.group(1)] = ddl_file

        # Indexes
        for m in re.finditer(
            r"CREATE\s+(?:UNIQUE\s+)?INDEX\s+(?:IF\s+NOT\s+EXISTS\s+)?(\w+)", text, re.IGNORECASE
        ):
            indexes[m.group(1)] = ddl_file

        # Views
        for m in re.finditer(
            r"CREATE\s+(?:OR\s+REPLACE\s+)?VIEW\s+(?:public\.)?(\w+)", text, re.IGNORECASE
        ):
        ...
        # Extensions
        for m in re.finditer(
            r'CREATE\s+EXTENSION\s+(?:IF\s+NOT\s+EXISTS\s+)?["\']?(\w+)', text, re.IGNORECASE
        ):
...
def main():
    ...
    print(
        f"Found: {len(tables)} tables, {len(functions)} functions, "
