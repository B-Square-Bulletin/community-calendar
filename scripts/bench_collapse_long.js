...
function main() {
  ...
  if (eventsIdx >= 0 && argv[eventsIdx + 1]) {
    ...
    if (!Array.isArray(events))
      throw new Error('--events file must contain a JSON array of events');
  } else {
  ...
  console.log(
    'events: ' +
      events.length +
      (eventsIdx >= 0 ? ' (real data: ' + argv[eventsIdx + 1] + ')' : ' (synthetic)')
  );
  console.log('');

  // Run #1 — cached emit. Cold cache: computes.
  const run1 = run(events);
  ...
  const run3 = run(JSON.parse(JSON.stringify(events)));
  console.log('run#3: ' + run3.ms.toFixed(1) + 'ms');

  console.log('');
  console.log('window._pipelineLog:');
  windowObj._pipelineLog.forEach(function (line) {
    console.log('  ' + line);
  });
  if (memoStats) {
    console.log(
      'window.__ccMemoStats.collapseLongRunningEvents: ' +
        memoStats.hits +
        ' hits / ' +
        memoStats.misses +
        ' misses'
  ...
  if (!run2Hit) {
    failures.push(
      'run#2 was not a cache HIT at either layer (content-key cache or issue-82 memoize)'
    );
  }
  ...
  console.log(
    'PASS — content-key cache verified: run#2 hits on fresh references at ' +
      run2.ms.toFixed(1) +
      'ms.'
  );
  process.exit(0);
