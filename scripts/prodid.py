...
PLATFORM_MAP = [
    ...
    (
        r"iCalendar-Ruby",
        "Localist",
        "Campus/community event platform by Concept3D (uses iCalendar-Ruby gem). Common at universities.",
    ),
...
def main():
    ...
    total_feeds = sum(sum(len(f) for f in p["cities"].values()) for p in platforms.values())
    print(
        f"{len(platforms)} platforms, {total_feeds} feeds across {len({c for p in platforms.values() for c in p['cities']})} cities"
    )

    if unclassified:
        total_unknown = sum(len(f) for files in unclassified.values() for f in files.values())
        print(
            f"{len(unclassified)} unclassified PRODIDs ({total_unknown} feeds) — see docs/prodid.md"
        )
