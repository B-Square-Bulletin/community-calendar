...
from backfill_scraper_feeds import workflow_scraper_rows  # noqa: E402
from feed_slug import slugify  # noqa: E402
from process_pending_feeds import parse_pending_feeds  # noqa: E402
from run_scrapers_from_db import load_scraper_rows as load_db_scraper_rows  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--months",
        default=os.environ.get("SCRAPE_MONTHS", "3"),
        help="SCRAPE_MONTHS value for scraper commands (default: 3)",
    )
    parser.add_argument(
        "--output-report",
        default="local-build-report.json",
        help="Path to structured audit report JSON",
    )
    parser.add_argument("--build-log", default="local-build.log", help="Path to build log")
    parser.add_argument(
        "--feed-report", default="local-feed-report.json", help="Path to feed health report JSON"
    )
    parser.add_argument(
        "--validation-report",
        default="local-validation-report.json",
...
def discover_all_cities() -> list[str]:
    return sorted(
        path.parent.name for path in ROOT.glob("cities/*/feeds.txt") if path.parent.is_dir()
    )
...
def query_feeds(
    city: str, feed_types: tuple[str, ...] | None = None, statuses: tuple[str, ...] | None = None
) -> list[dict] | None:
...
def summarize_output(path: Path) -> dict:
    ...
    summary = {
        ...
        "events": events,
        "status": ("not_ics" if not is_calendar else "no_events" if events == 0 else "ok"),
    }
...
def build_scraper_drift(
    city: str, workflow_rows: list[dict], db_rows: list[dict] | None
) -> list[dict]:
    if db_rows is None:
        return []
    ...
    for url, workflow_row in workflow_by_url.items():
        ...
        if (
            db_row.get("name") != workflow_row["name"]
...
def run_city(city: str, logger: BuildLogger, args: argparse.Namespace) -> dict:
    ...
    snapshot_rows = [
        ...
        if row["feed_type"] in {"ics_url", "curator", "scraper"}
    ...
    with maybe_db_export():
        for row in execution_rows:
            ...
            if result["returncode"] != 0:
                scraper_entry["failure_type"] = classify_command_failure(
        ...
        if feeds_path.exists():
            from download_feeds import (
                parse_feeds_txt,
            )  # local import to avoid circular startup cost

            feed_inventory = list(parse_feeds_txt(feeds_path))
        ...
        geo_report_path = (
            Path(tempfile.mkdtemp(prefix=f"local-build-geo-{city}-")) / "geo_filtered.json"
        )
        combine_result = run_command(
            ...
            [
                ...
                "--name",
                f"{titleize_city(city)} Community Calendar",
                "--geo-report",
                str(geo_report_path),
            ],
            env,
        ...
        rss_result = run_command(
            ...
            city,
            "rss",
            [sys.executable, "scripts/generate_rss.py", city, "--outdir", str(rss_outdir)],
            env,
        )
        city_result["rss"] = {
...
def summarize_city_result(city_result: dict) -> dict:
    ...
    scraper_missing = sum(
        1 for row in city_result["scrapers"] if row["output"]["status"] == "missing"
    )
    scraper_zero_event = sum(
        1 for row in city_result["scrapers"] if row["output"]["status"] == "no_events"
    )
    scraper_not_ics = sum(
        1 for row in city_result["scrapers"] if row["output"]["status"] == "not_ics"
    )
    live_feed_missing = sum(
        1 for row in city_result["live_feeds"] if row["output"]["status"] == "missing"
    )
    live_feed_zero_event = sum(
        1 for row in city_result["live_feeds"] if row["output"]["status"] == "no_events"
    )
    live_feed_not_ics = sum(
        1 for row in city_result["live_feeds"] if row["output"]["status"] == "not_ics"
    )
    return {
            ...
            actions["retire"].append(
                {
                    ...
                    "name": issue["name"],
                    "path": issue["url"],
                    "message": (f"Remove retired scraper {issue['name']} from the workflow."),
                }
            )
            ...
            actions["fix"].append(
                {
                    ...
                    "message": (f"Scraper {scraper['name']} failed or did not produce output."),
...
def main() -> int:
    ...
    for city_result in results:
        city_result["build_issues"] = [
            issue for issue in build_errors if issue.get("city") == city_result["city"]
        ]
    ...
    audit = {
        ...
        "summary": {
            ...
            "scraper_missing_outputs": sum(
                city["summary"]["scraper_missing_outputs"] for city in results
            ),
            "scraper_zero_event_outputs": sum(
                city["summary"]["scraper_zero_event_outputs"] for city in results
            ),
            "scraper_not_ics_outputs": sum(
                city["summary"]["scraper_not_ics_outputs"] for city in results
            ),
            "live_feed_missing_outputs": sum(
                city["summary"]["live_feed_missing_outputs"] for city in results
            ),
            "live_feed_zero_event_outputs": sum(
                city["summary"]["live_feed_zero_event_outputs"] for city in results
            ),
            "live_feed_not_ics_outputs": sum(
                city["summary"]["live_feed_not_ics_outputs"] for city in results
            ),
            ...
            "build_warning_count": sum(
                1 for issue in build_errors if issue.get("level") == "warning"
            ),
            "runtime_issue_count": len(runtime["issues"]),
            "runtime_error_count": sum(
                1 for issue in runtime["issues"] if issue.get("level") == "error"
            ),
            "runtime_warning_count": sum(
                1 for issue in runtime["issues"] if issue.get("level") == "warning"
            ),
