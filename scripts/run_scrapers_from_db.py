...
def query_db_scraper_rows(city: str) -> tuple[list[dict] | None, str | None]:
    ...
    for row in raw:
        if not row.get("scraper_cmd"):
            ...
            print(
                f"[db-first] WARNING skipping command-less scraper row id={row.get('id')} {row.get('name')}"
            )
            continue
        rows.append(
            {
...
def main() -> int:
    ...
    parser.add_argument(
        "--list", action="store_true", help="List the execution set without running anything"
    )
    ...
    print(
        f"[db-first] city={args.city} ran={len(rows)} failures={failures} "
