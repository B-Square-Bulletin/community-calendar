...
def _serialize_vevent(event, original_had_rrule):
    ...
    if original_had_rrule:
        ...
        content = re.sub(
            r"^(RRULE|EXDATE|RDATE|RECURRENCE-ID)[;:][^\r\n]*\r?\n?",
            "",
            content,
            flags=re.MULTILINE,
        )
    ...
    for key, group in groups.items():
        if len(group) == 1:
            ...
            group.sort(
                key=lambda e: (
                    1
                    if is_aggregator(extract_field(e["content"], "X-SOURCE") or "")
                    else 0
                )
            )
            ...
            if source_urls:
                kept["content"] = (
                    f"X-SOURCE-URLS:{json.dumps(source_urls)}\r\n{kept['content']}"
                )
...
JSON:"""
            ...
            for cluster in clusters:
                ...
                cluster_events.sort(
                    key=lambda x: (
                        1
                        if is_aggregator(
                            extract_field(x[1]["content"], "X-SOURCE") or ""
                        )
                        else 0
                    )
                )
                ...
                kept_title = (
                    extract_field(kept_event["content"], "SUMMARY") or "(no title)"
                )
                kept_source = (
                    extract_field(kept_event["content"], "X-SOURCE") or "Unknown"
                )
                ...
                for idx, removed_event in cluster_events[1:]:
                    removed_title = (
                        extract_field(removed_event["content"], "SUMMARY")
                        or "(no title)"
                    )
                    removed_source = (
                        extract_field(removed_event["content"], "X-SOURCE") or "Unknown"
                    )
    ...
    print(
        f"  Fuzzy dedup: {api_calls} API calls, {total_input_tokens}+{total_output_tokens} tokens"
    )

    if fuzzy_deduped > 0:
        print(
            f"  Fuzzy dedup: removed {fuzzy_deduped} duplicate events (see fuzzy_dedup.log)"
        )
    else:
        print("  Fuzzy dedup: no additional duplicates found")
...
def combine_ics_files(
    ...
    for ics_file in sorted(ics_dir.glob("*.ics")):
        ...
        try:
            ...
            future_events = [
                e for e in events if e["dtstart"].replace(tzinfo=timezone.utc) >= now
            ]
            ...
            future_events = [
                e for e in future_events if not _url_predates_window(e["content"], now)
            ]
            url_date_filtered = before_filter - len(future_events)
            if url_date_filtered:
                print(
                    f"    Filtered {url_date_filtered} events with stale URL dates from {ics_file.name}"
                )
            ...
            if allowed_cities:
                ...
                for e in future_events:
                    ...
                    if location_matches_allowed_cities(
                        location, allowed_cities, excluded_cities
                    ):
            ...
            if future_events:
                ...
                actual_source = (
                    extract_field(future_events[0]["content"], "X-SOURCE")
                    or source_name
                )
                print(
                    f"  {len(future_events):4d} future events from {ics_file.name} ({actual_source})"
                )
        except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-dir", "-i", required=True, help="Directory containing ICS files"
    )
    parser.add_argument("--output", "-o", required=True, help="Output ICS file")
    parser.add_argument(
        "--name", "-n", default="Community Calendar", help="Calendar name"
    )
    parser.add_argument(
        "--exclude",
        "-x",
