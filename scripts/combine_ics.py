...
JSON:"""
        ...
        try:
            ...
            for cluster in clusters:
                ...
                cluster_events = [
                    (idx - 1, day_events[idx - 1]) for idx in cluster if 0 < idx <= len(day_events)
                ]
                ...
                cluster_events.sort(
                    key=lambda x: (
                        1 if is_aggregator(extract_field(x[1]["content"], "X-SOURCE") or "") else 0
                    )
                )

                # Keep first, merge sources from rest, then mark rest for removal
                _kept_idx, kept_event = cluster_events[0]
                ...
                for idx, removed_event in cluster_events[1:]:
                    removed_title = (
                        extract_field(removed_event["content"], "SUMMARY") or "(no title)"
                    )
                    removed_source = (
            ...
            if dt:
                ...
                if source_name:
                    ...
                    if evt_url and "X-SOURCE-URLS" not in event_content:
                        event_content = (
                            f"X-SOURCE-URLS:{json.dumps({source_name: evt_url})}\r\n{event_content}"
                        )
    ...
    for ics_file in sorted(ics_dir.glob("*.ics")):
        ...
        try:
            ...
            if future_events:
                ...
                actual_source = (
                    extract_field(future_events[0]["content"], "X-SOURCE") or source_name
                )
                print(
