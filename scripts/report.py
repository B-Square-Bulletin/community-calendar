...
def update_report(cities: list[str], report_path: str = "report.json"):
    ...
    existing_today = {
        (a["city"], a["feed"], a["type"]) for a in report["anomalies"] if a.get("date") == today
    }
    ...
    for city in cities:
        ...
        unique_urls = len({e["url"] for e in urls_with_url})
        ...
        for domain, info in by_domain.items():
            if len(info["urls"]) == 1 and info["count"] > 5:
                generic_domains.append(
                    {"domain": domain, "events": info["count"], "url": next(iter(info["urls"]))}
                )
    ...
    for city in cities:
        ...
        for src, entries in events_by_source.items():
            ...
            if daytime >= len(suspicious) * 0.7:
                samples = []
                for e, sh in zip(suspicious[:5], shifted[:5], strict=False):
                    samples.append(
                        {
