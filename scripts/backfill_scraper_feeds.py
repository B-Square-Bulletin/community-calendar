...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Print planned inserts without writing"
    )
    parser.add_argument(
        "--sync-existing",
        action="store_true",
...
def workflow_scraper_rows(city_filter: str | None = None) -> list[dict[str, str]]:
    ...
    for raw_line in WORKFLOW_PATH.read_text().splitlines():
        ...
        if not (
            script_path.startswith("scrapers/") or script_path == "scripts/library_intercept.py"
        ):
            continue
...
def fetch_existing_rows(
    supabase_url: str, service_key: str, city_filter: str | None = None
) -> list[dict]:
    query = (
...
def build_sync_plan(
    ...
    for row in workflow_rows:
        ...
        if canonicalize_scraper_cmd(existing.get("scraper_cmd")) != canonicalize_scraper_cmd(
            row["scraper_cmd"]
        ):
...
def apply_sync_plan(
    plan: dict[str, list[dict]], supabase_url: str, service_key: str
    ...
    for row in plan["update"]:
        try:
            ...
            updated += 1
            print(
                f"~ {row['city']}: {row['name']} [{row['url']}] -> {json.dumps(row['payload'], sort_keys=True)}"
            )
        except urllib.error.HTTPError as exc:
            errors += 1
            detail = exc.read().decode()
...
def main() -> int:
    ...
    for row in plan["update"]:
        print(
            f"  UPDATE {row['city']}: {row['name']} [{row['url']}] -> {json.dumps(row['payload'], sort_keys=True)}"
        )
    for row in plan["retire"]:
    ...
    assert supabase_url and service_key  # non-dry-run requires credentials (see above)
