...
def validate_scraper_health(city_result: dict) -> list[ValidationError]:
    ...
    for scraper in city_result.get("scrapers", []):
        ...
        if scraper["returncode"] != 0:
            errors.append(
                ValidationError(
                    "error", city, f"{name}: command failed with exit {scraper['returncode']}"
                )
            )
        ...
        if output_status == "not_ics":
            ...
            errors.append(
                ValidationError(
                    "error", city, f"{name}: output is not ICS ({content_kind}) despite exit 0"
                )
            )
        if error_issues:
            errors.append(
                ValidationError("error", city, f"{name}: {len(error_issues)} logged error(s)")
            )
        if warning_issues:
            errors.append(
                ValidationError("warning", city, f"{name}: {len(warning_issues)} logged warning(s)")
            )
        ...
        try:
            ...
            for critical in CRITICAL_SOURCES.get(city, []):
                if critical not in sources:
                    errors.append(
                        ValidationError("warning", city, f"Critical source missing: {critical}")
                    )
            ...
            if len(sources) < 3:
                errors.append(
                    ValidationError(
                        "warning", city, f"Low source diversity: only {len(sources)} sources"
                    )
                )
...
def main():
    ...
    parser.add_argument(
        "--cities", "-c", required=True, help="Comma-separated list of cities to validate"
    )
    parser.add_argument(
        "--strict", action="store_true", help="Treat warnings as errors (exit non-zero)"
    )
    parser.add_argument("--cities-dir", default="cities", help="Path to cities directory")
