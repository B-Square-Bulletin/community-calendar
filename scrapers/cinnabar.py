    ...
    def _parse_date_range(cls, text: str) -> tuple:
        ...
        if m:
            ...
            if start_month and end_month:
                try:
                    dtstart = wall_clock(year, start_month, int(m.group(2)))
                    # A range like 'Dec. 19 - Jan. 4 2027' wraps into the next year
                    end_year = year + 1 if end_month < start_month else year
                    dtend = wall_clock(end_year, end_month, int(m.group(4)))
        ...
        if m:
            ...
            if month:
                try:
                    year = int(m.group(4))
                    dtstart = wall_clock(year, month, int(m.group(2)))
                    dtend = wall_clock(year, month, int(m.group(3)))
                    return dtstart, dtend
                except ValueError:
                    pass
        ...
        if m:
            ...
            if month:
                try:
                    dtstart = wall_clock(int(m.group(3)), month, int(m.group(2)))
                    return dtstart, None
                except ValueError:
                    pass
