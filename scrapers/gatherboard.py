...
class GatherBoardScraper(BaseScraper):
    ...
    def _discover_event_paths(self) -> list[str]:
        ...
        while next_url and page_num < max_pages:
            ...
            new_links = re.findall(r'href=["\'](\d{2}/\d{2}/\d{4}/[^"\'>\s]+)["\']', html)
    ...
    def _fetch_event_from_detail(self, path: str) -> dict[str, Any] | None:
        ...
        if not ics_match:
            ...
            ics_match = re.search(r'href=["\'](/[a-f0-9]+/ical/)["\']', html, re.IGNORECASE)
            if ics_match:
    ...
    def _parse_ics(self, ics_content: str, fallback_url: str) -> dict[str, Any] | None:
...
def main():
    ...
    parser.add_argument(
        "--url", required=True, help="Site base URL (e.g. https://www.sonomavalleyevents.com)"
    )
