...
class SeeTicketsScraper(BaseScraper):
    ...
    def fetch_events(self) -> list[dict[str, Any]]:
        ...
        response = requests.get(
            self.events_url, headers=HEADERS, timeout=30, verify=self.verify_ssl
        )
    ...
    def _parse_card(self, card, now: datetime, tz: ZoneInfo) -> dict[str, Any] | None:
        ...
        location = (
            venue_el.get_text(strip=True).lstrip("at ") if venue_el else self.default_location
        )
        if not location:
    ...
    @staticmethod
    def _parse_datetime(
        date_text: str, time_text: str, now: datetime, tz: ZoneInfo
    ) -> datetime | None:
