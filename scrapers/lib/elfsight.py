...
def fetch_elfsight_data(widget_id: str, source_page: str) -> dict | None:
    """
...
def expand_recurring_events(
    event: dict, months_ahead: int = 3, max_occurrences: int = 500
) -> list[datetime]:
    """
    ...
    repeat_ends_date = (
        event.get("repeatEndsDate", {}).get("date") if event.get("repeatEndsDate") else None
    )
...
class ElfsightCalendarScraper(BaseScraper):
    ...
    def fetch_settings(self) -> dict | None:
        ...
        if self._settings is None:
            ...
            if self._settings:
                # Build lookup maps
                self._locations_map = {
                    loc["id"]: loc["name"] for loc in self._settings.get("locations", [])
                }
                self._event_types_map = {
                    et["id"]: et["name"] for et in self._settings.get("eventTypes", [])
                }
                self.logger.info(
    ...
    def _matches_filter(self, values: list[str], filters: list[str]) -> bool:
        ...
        return any(filt in val for val in values_lower for filt in filters_lower)
    ...
    def fetch_events(self) -> list[dict]:
        ...
        for event in raw_events:
            ...
            if occurrences:
                self.logger.debug(f"  {name}: {len(occurrences)} occurrences @ {location_str}")

        self.logger.info(f"Expanded to {len(parsed_events)} event occurrences")
        return parsed_events

    @classmethod
    def parse_args(cls, description: str | None = None):
        ...
        parser.add_argument(
            "--location", "-l", nargs="+", default=[], help="Filter to specific location(s)"
        )
        parser.add_argument(
            "--type", "-t", nargs="+", default=[], help="Filter to specific event type(s)"
        )
        parser.add_argument(
            "--list-locations", action="store_true", help="List available locations and exit"
        )
        parser.add_argument(
            "--list-types", action="store_true", help="List available event types and exit"
        )
        parser.add_argument(
            "--months",
            "-m",
