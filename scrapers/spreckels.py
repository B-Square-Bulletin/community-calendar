...
class SpreckelsScraper(BaseScraper):
    ...
    SEASON_LINK_RE = re.compile(r"https://spreckelsonline\.com/\d{4}-\d{2}-season/")
    ...
    def _parse_show(self, url: str, season_years: tuple[int, int]) -> dict | None:
        html = self._get(url)
        ...
        paragraphs = [
            p.get_text(strip=True).replace("–", "-").replace("—", "-")
    ...
    @classmethod
    def _parse_month_day(cls, text: str) -> tuple[int, int] | None:
