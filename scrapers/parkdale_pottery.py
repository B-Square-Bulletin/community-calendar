#!/usr/bin/env python3
"""Parkdale Pottery workshops from Shopify products."""
...
TIME_RANGE_RE = re.compile(
    r"(?:from|times?\s*:)\s+(?P<start>\d{1,2}(?::\d{2})?(?:\s*(?:am|pm))?)\s*-\s*"
    r"(?P<end>\d{1,2}(?::\d{2})?\s*(?:am|pm))",
    re.IGNORECASE,
)
LOCATION_RE = re.compile(r"Location\s*:\s*(?P<location>[^*]+)", re.IGNORECASE)
...
class ParkdalePotteryScraper(BaseScraper):
    name = "Parkdale Pottery"
    domain = "parkdalepottery.ca"
    timezone = "America/Toronto"
    base_url = "https://parkdalepottery.ca"
    ...
    def fetch_events(self) -> list[dict]:
        ...
        for product in data.get("products", []):
            if product.get("product_type") != "Workshop":
                continue
            ...
            if location_match:
                location_text = location_match.group("location").split("Instructor")[0].strip(" -:")
                if location_text:
