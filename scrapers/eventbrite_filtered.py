        ...
        if self.report:
            ...
            if out_of_area:
                ...
                for v in sorted(set(out_of_area))[:30]:
                    self.logger.info(f"  {v}")
            if passed_venues:
                self.logger.info("--- PASSED VENUES (unique, first 50) ---")
...
def main():
    ...
    parser.add_argument(
        "--geo-allowlist",
        help="Path to geo allowlist file (require LOCATION to contain at least one keyword)",
    )
    parser.add_argument("--name", required=True, help="Source display name (X-SOURCE)")
    parser.add_argument("--output", "-o", help="Output ICS file")
    parser.add_argument(
        "--report", action="store_true", help="Print denied + out-of-area + passed venue samples"
    )
