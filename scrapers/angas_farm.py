#!/usr/bin/env python3
"""
Scraper for Anga's Farm & Nursery events.

Squarespace site with per-event ICS files.
Strategy: scrape /events-2 listing for event slugs, fetch each ?format=ical.

Usage:
    python scrapers/angas_farm.py --output cities/toronto/angas_farm.ics
"""
...
class AngasFarmScraper(BaseScraper):
    """Scraper for Anga's Farm & Nursery via Squarespace per-event ICS."""

    name = "Anga's Farm & Nursery"
    domain = "angasfarm.ca"
