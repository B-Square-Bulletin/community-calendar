...
def parse_time_range(text: str) -> tuple[tuple[int, int] | None, tuple[int, int] | None]:
    """
...
def scrape_events(target_year: int, target_month: int) -> list[dict]:
    ...
    if "2026 weekend" in page_text.lower() or "weekend afternoon" in page_text.lower():
        ...
        dates_match = re.search(
            r"(sat(?:urdays?)?,?\s*)?(mar(?:ch)?\s+\d{1,2})[;,]?\s*(june?\s+\d{1,2})[;,]?\s*(sep(?:t(?:ember)?)?\s+\d{1,2})[;,]?\s*(dec(?:ember)?\s+\d{1,2})",
            page_text.lower(),
        )
        if dates_match:
            ...
            for dt in workshop_dates:
                if dt.month == target_month:
                    event = {
                        ...
                        "dtstart": dt.replace(hour=start_time[0], minute=start_time[1])
                        if start_time
                        else dt,
                        "dtend": dt.replace(hour=end_time[0], minute=end_time[1])
                        if end_time
                        else dt + timedelta(hours=3),
                        "location": LOCATION,
                        "url": WORKSHOPS_URL,
                    }
                    events.append(event)
    ...
    if "first friday" in page_text.lower() and "women" in page_text.lower():
        ...
        circle_dates = parse_recurring_monthly(
            "first friday of the month", target_year, target_month
        )
...
def create_calendar(events: list[dict], year: int, month: int) -> Calendar:
    ...
    for event_data in events:
        ...
        naive_end = (
            dtend.replace(tzinfo=None) if hasattr(dtend, "tzinfo") and dtend.tzinfo else dtend
        )
        ...
        description = append_source(
            event_data.get("description", ""), event_data.get("url", WORKSHOPS_URL)
        )
...
def main():
    ...
    parser.add_argument(
        "--month", type=int, help="Target month 1-12 (default: current + next month)"
    )
