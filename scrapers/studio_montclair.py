...
def _parse_month_day_year(text: str, default_year: int = 2026) -> datetime | None:
...
class StudioMontclairScraper(BaseScraper):
    ...
    def _extract_events_from_post(self, post: dict) -> list[dict[str, Any]]:
        ...
        for pm in prog_matches:
            year = (
                int(pm.group(2)) if pm.group(2) else (end_dt.year if exh_match and end_dt else 2026)
            )
            ...
            time_match = re.search(
                r"(?:begins? at|at)\s*(\d{1,2})(?::(\d{2}))?\s*(pm|am)", after_text, re.IGNORECASE
            )
            hour = 19  # default 7pm
            if time_match:
            ...
            topic_match = re.search(
                r"((?:Panel|Discussion|Talk|Lecture|Workshop)[^.]*)", before_text, re.IGNORECASE
            )
