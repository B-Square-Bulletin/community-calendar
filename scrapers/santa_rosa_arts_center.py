...
def parse_event_date(text: str, target_year: int) -> datetime | None:
    """
    Parse dates like:
    ...
    match = re.search(
        r"(january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|oct|nov|dec)\s+(\d{1,2})",
        text,
    )
    if match:
    ...
    for widget in content_widgets:
        ...
        for element in elements:
            ...
            if element.name == "h3":
                ...
                current_event = {"title": title, "description": "", "url": EVENTS_URL}
                continue
            ...
            if element.name == "h5" and current_event:
                ...
                if date and not current_event.get("dtstart"):
                    ...
                    if start_time:
                        current_event["dtstart"] = date.replace(
...
def create_calendar(events: list[dict], year: int, month: int) -> Calendar:
    ...
    for event_data in events:
        ...
        description = append_source(
            event_data.get("description", ""), event_data.get("url", EVENTS_URL)
        )
