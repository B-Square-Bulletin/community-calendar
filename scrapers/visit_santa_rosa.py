...
class VisitSantaRosaScraper(BaseScraper):
    ...
    def _extract_algolia_credentials(self) -> tuple[str | None, str | None]:
    ...
    def _query_algolia(self, app_id: str, api_key: str, page: int = 0) -> dict | None:
    ...
    def fetch_events(self) -> list[dict[str, Any]]:
        ...
        while True:
            ...
            self.logger.info(
                f"Page {page}/{nb_pages - 1}, {len(hits)} hits (total so far: {len(all_hits)})"
            )
            if page >= nb_pages - 1:
                break
            page += 1
    ...
    def _parse_hit(self, hit: dict, now: datetime) -> dict[str, Any] | None:
        ...
        location = ", ".join(str(p) for p in address_parts if p) if address_parts else ""
