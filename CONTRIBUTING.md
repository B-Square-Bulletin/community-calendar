...
- **Pre-commit**: enable once with `pre-commit install` (uses the `pre-commit` framework, not Husky). Python hooks rewrite staged files (`ruff format`, `ruff check --fix`); JS hooks are check-only on staged files (`prettier --check` and `eslint`). Fix with `make format` (`format-js`/`format-python` for one language); full gate with `make lint`.
