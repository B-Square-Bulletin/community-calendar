...
- **Pre-commit**: enable once with `pre-commit install` (uses the `pre-commit` framework, not Husky). Hooks are check-only on staged files: ruff (Python) plus `prettier --check` and `eslint` (JS). Fix with `make format` (`format-js`/`format-python` for one language); full gate with `make lint`.
